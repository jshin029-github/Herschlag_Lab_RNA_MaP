classdef DisplayFun
    methods (Static)
        ...
        function f = progressBar(totalIterations)
            barLength = 30;
            startTime=datenum(clock);
            totalCharLength = 0;        
            f = @updateBar;
                function updateBar(currIteration, updateInterval)
                    %if we are at an interval where we want to update the
                    %progress bar
                    if (mod(currIteration,updateInterval) == 0) || (currIteration == 1) || (currIteration == totalIterations) 

                        % calculate percent complete
                        percentComplete = currIteration/totalIterations;
                        
                        % compute time info
                        elapsedTime = datenum(clock)-startTime;
                        [elapsedYears, elapsedMonths, elapsedDays, elapsedHours, elapsedMinutes, elapsedSeconds] = datevec(elapsedTime);
                        timePerCycle = elapsedTime/currIteration;
                        [remainingYears, remainingMonths, remainingDays, remainingHours, remainingMinutes, remainingSeconds] = datevec(timePerCycle*(totalIterations-currIteration));

                        % remove previous bar with backspace characters
                        fprintf('%s',repmat(char(8),1,totalCharLength)); 

                        % assemble progress bar string
                        barElapsed = round(barLength*percentComplete);
                        barRemaining = barLength - barElapsed;
                        progressBarString = sprintf('[%s%s]',repmat('#',1,barElapsed),repmat('-',1,barRemaining));
                        lengthProgressBarString = length(progressBarString);
                        
                        % assemble info string
                        infoString = sprintf(' %3.1f%% - elapsed: %02dh:%02dm:%02ds, remaining: %02dh:%02dm:%02ds',percentComplete*100,elapsedHours,elapsedMinutes,round(elapsedSeconds),remainingHours,remainingMinutes,round(remainingSeconds));
                        lengthInfoString = length(infoString);
                        totalCharLength = lengthProgressBarString + lengthInfoString;
                        
                        % write progress bar
                        fprintf('%s',[progressBarString, infoString]); 
                    end
                end
        end %END progressBar
        ...
        function displayHistogramASCII(N, X)
            max_counts = max(N); % how many counts does the histogram bin with the most counts in it have?
            total_counts = sum(N);

            max_characters_in_ASCII_bar = 30; % we will display the histogram bin with the most counts using this many ASCII characters

            num_bins = length(N);

            disp('bin center / ASCII bin count / bin count / bin percentage of total');
            for idx_bin = 1:num_bins
                normalized_counts = N(idx_bin) / max_counts;
                num_characters_to_display = round(normalized_counts * max_characters_in_ASCII_bar);
                percentage_counts = round(100 * N(idx_bin) / total_counts);
                str = repmat('X', 1, num_characters_to_display);
                str = [str repmat(' ', 1, max_characters_in_ASCII_bar - num_characters_to_display)];
                str = sprintf('% 15g %s   %10g   %d%%', X(idx_bin), str, N(idx_bin), percentage_counts);
                disp(str)
            end
        end
        ...
        if nargin < 1
            disp('txtmenu requires at least one input');
        end
